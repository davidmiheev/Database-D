...
int main() {
    ...
    f = fopen("e.dat","r");
    BTree tree;
    char key[256],name[NAME_LEN],info[INFO_LEN]; int n,i = 0;
        cout << "\n+------------+\n";
        cout <<   "| Database D |\n";
        cout <<   "+------------+\n\n";
    while(1) {
        ...
        if(strcmp(key,"add") == 0) { name[0] = info[0] = key[0] = '\0'; 
            cout << "Enter student's name: ";
            while(i == 0 || i == 1 || (name[i - 1] != '\n' && i < NAME_LEN)) {
                scanf("%c", &name[i]); 
                i++; 
            } name[i - 1] = '\0'; for(int j = 0;j < i - 1;j++) name[j] = name[j + 1]; i = 0;
            ...
            Student s(name,n,info); tree.add(s,n);  tree.showtree(); cout << endl; /*tree.print(tree.root());*/ }
        if(strcmp(key,"search") == 0) { name[0] = key[0] = '\0';
            cout << "Enter student's name: ";
            while(i == 0 || i == 1 || (name[i - 1] != '\n' && i < NAME_LEN)) {
                scanf("%c", &name[i]);
                i++;
            ...
            if(tree.search(tree.root(),n,&res)) {
                ...
                for (m = res->n - 1, t = 0; m - t > -1;)
                    {
                        if (res->value[(int)((t + m)/2)].x == n) { if(!res->value[(int)((t + m)/2)].y.search(name)) cout << "student not found\n" << endl; break; }
        ...
        if(strcmp(key,"search_by_num") == 0) { key[0] = '\0';
            cout << "Enter number of this student's group (without spaces): ";
            cin >> n;
            cout << endl;
            BTreeNode *res;
            if(tree.search(tree.root(),n,&res)) {
                ...
                for (m = res->n - 1, t = 0; m - t > -1;)
                    {
                        if (res->value[(int)((t + m)/2)].x == n) { res->value[(int)((t + m)/2)].y.printFront(); break; }
                        if (res->value[(int)((t + m)/2)].x > n) m = (int) (m + t)/2 - 1;
        ...
        if(strcmp(key,"del_by_num") == 0) { key[0] = '\0'; cout << "Enter number of group (without spaces): "; cin >> n; tree.del(n); tree.showtree(); cout << endl; }
        if(strcmp(key, "del") == 0) { name[0] = key[0] = '\0';
            cout << "Enter student's name: ";
            while(i == 0 || i == 1 || (name[i - 1] != '\n' && i < NAME_LEN)) {
                scanf("%c", &name[i]);
                i++;
