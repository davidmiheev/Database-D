...
public:
    ...
    { strcpy(name,s); strcpy(info,inf); }
    
    Student(const Student& s):
        name(new char[strlen(s.name) + 1]),
        num_group(s.num_group),
        info(new char[strlen(s.info) + 1])
    { strcpy(name,s.name); strcpy(info,s.info); }
    
    Student& operator=(const Student& s) { if(this != &s) { if(name) delete[] name; if(info) delete[] info; name = new char[strlen(s.name) + 1]; 
        num_group = s.num_group;
    ...
    bool operator>(const Student& b) {
        if(strcmp(name,b.get_name()) > 0) return true;
        else return false;
    }
    
    bool operator<=(const Student& b) { return !(*this > b); }
    
    bool operator==(const Student& b) { 
      if(strcmp(name,b.get_name()) == 0 && num_group == b.get_num()) return true;
        else return false;
    }
...
public:
    ...
    void AddAfter(const char *s,const int n,const char *info) {
        ...
        if(cur == 0x00) first -> next = cur = i;
        else
       {
         i->next = cur->next;
         cur->next = i;
         if(i->next == 0x00) cur = i;
       }
    }
    
    void print() {
        int err = 1; Student v;
        for(GotoFirst(), err = IsEmpty(); !err; err = GotoNext()) {GetCur(&v); v.print();}
        cout << endl;
    }
...
public:
    ...
    /*ListDeq(int maxElem):
        ...
        m_End(maxElem-1),
        m_NumElem(0),
        m_Elements(new SList[m_MaxElem])
    {
    }*/
    ...
    { for(int i = 0; i < DEQ_MAXELEM; i++) m_Elements[i] = d.m_Elements[i]; }
...
public:
    ...
    void add(const Student& s) {
      if(m_NumElem == 0) { SList *l = new SList; pushFront(l); front()->AddAfter(s.get_name(),s.get_num(),s.get_info()); }
      else {
    ...
    void clear() {
        if(m_NumElem > 0) { for(int i = m_Begin; i <= m_End; i++) delete m_Elements[i]; }
        m_Begin = 0;
        m_End = m_MaxElem-1;
        m_NumElem = 0;
    }
...
public:
    ...
    void printkey() { cout << x << " "; }
    void print() { cout << "+--------+" << endl; y.printFront(); cout << "key: " << x << endl; cout << "+----------+" << endl; }
    void clear() { x = 0; y.clear(); } 
...
public:
    ...
    BTreeNode(): 
        parent(0),
        n(0)
    { for(int i = 0; i < 2*BTREE_DEG + 1; i++) son[i] = 0; }
    
    ~BTreeNode() { parent = 0; n = 0; for(int i = 0; i < 2*BTREE_DEG + 1; i++) son[i] = 0;  }
...
private:
        int binsearch(BTreeNode *node, int key) {
            ...
            {
                if (node->value[(int)((t + m)/2)].x == key) return -1;
                
                if (node->value[(int)((t + m)/2)].x > key) m = (int) (m + t)/2 - 1;

                if (node->value[(int)((t + m)/2)].x < key) t = (int) (m + t)/2 + 1; 
            }
             return t;
        }
        ...
        void split(BTreeNode *node) {
                    ...
                    for(int j = 0; j < BTREE_DEG; j++) n_1->son[j] = node -> son[j];
                    for(int j = BTREE_DEG; j < 2*BTREE_DEG + 1; j++) n_2->son[j - BTREE_DEG] = node -> son[j];
                    for(int j = 0; j < BTREE_DEG; j++) { if(n_1->son[j]) n_1->son[j]->parent = n_1; }
                    for(int j = BTREE_DEG; j < 2*BTREE_DEG + 1; j++) { if(n_2->son[j - BTREE_DEG]) n_2->son[j - BTREE_DEG]->parent = n_2; }
        ...
        void insert(BTreeNode *node, Pair &pair) {
                ...
                if((node->n) >= BTREE_DEG || node == header->son[0]) {
                    if(node->son[0] == 0x00) {
                        ...
                        switch(i) {
                            ...
                            case 1:
                                tmp = node->son[k];
                                while(tmp->son[0]) tmp = tmp->son[tmp->n];
            ...
            void join(BTreeNode *node, int k) {
              ...
              for(int j = node->son[k]->n; j < node->son[k + 1]->n + node->son[k]->n + 1; j++) node->son[k]->son[j] = node->son[k + 1]->son[j-node->son[k]->n];
...
public:       
            ...
            void showtree(/*BTreeNode *subroot*/) {
                if(header->son[0] == 0) cout << "---" << endl;
                else {
                int h = 0,level = 0; BTreeNode *tmp = root();
                while(tmp->son[0] != 0) { h++; tmp = tmp->son[0]; } 
                vector<int> *v = new vector<int>[h + 1],u(SIZE_BUFFER); 
                for(level = 0;level <= h;level++)  { v[level] = u; levelprint(root(),level,&v[level]); } 
                for(level = 0;level <= h;level++) { 
                    if(level == 0)  cout << "+--" << "---B-tree visualisation---" << "--+\n" << endl;
                    for(int i = 0; v[level][i] != 0; i += v[level][i] + 1) { cout << "( ";
                     for(int j = 0; j < v[level][i]; j++) { if(j < v[level][i] - 1) cout << v[level][j + i + 1] << " | "; else cout << v[level][j + i + 1] << " )  ";}
                    } if(level < h) cout << endl << endl;
                    else  cout << endl << "\n+--" << "--------------------------" << "--+" << endl;        
                    } delete[] v;
                }
            }
            ...
            void levelprint(BTreeNode *subroot,int level,vector<int> *v) {
                if(level == 0) { int j; for(j = 0;v->at(j) != 0; j += v->at(j) + 1);
                    for(int i = j; i < j + subroot->n + 1; i++) { if(i == j) v->at(i) = subroot->n; else v->at(i) = subroot->value[i - j - 1].x; } 
                } 
                else { level--;
                    for(int i = 0; i < subroot -> n + 1; i++) levelprint(subroot->son[i],level,v);
                }
            }
                       
            void print(BTreeNode *subroot) {
                
                if(subroot == 0x00) { cout << "null" << endl; return; }
                for(int i = 0; i < subroot -> n; i++) {
                     //subroot -> son[1] -> value[0].print();
                    subroot->value[i].print(); //
                    if(i == 0) print(subroot -> son[0]);
            ...
            void clear(BTreeNode *subroot) {
                if(subroot == 0x00) return;
                for(int i = 0; i < subroot -> n; i++) {
                    subroot->value[i].clear(); 
                    if(i == 0) clear(subroot -> son[0]);
            ...
            BTreeNode* root() { return header->son[0]; }
            
private:            
            void clean(BTreeNode *subroot,int k) {
                if(subroot->son[0] == 0x00) { BTreeNode *tmp = subroot; subroot->parent->son[k] = 0x00; --numNode;  delete tmp;   }
                else {
                for(int i = subroot->n; i >= 0; i--) {
                    if(subroot->son[i] != 0x00) clean(subroot->son[i],i);
                  }
                }
            }
...
/*{
 * 
 * //if(i > (int)((node->n)/2))  insert(n_2,pair); 
   //else  insert(n_1,pair);
 * 
 */
